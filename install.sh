...
install_ccproxy() {
  ...
  if [[ ! -x "$ccproxy_exe" ]]; then
    ...
    # Post-install check: detect if binary has no auth provider plugins
    # (known upstream bug in v0.2.10 Windows/Linux builds)
    if ! test_ccproxy_has_providers "$ccproxy_exe"; then
      warn "CCProxy binary has no auth provider plugins (known upstream bug)."
      warn "Upstream: https://github.com/CaddyGlow/ccproxy-api/issues/75"
      if test_pipx_available; then
        info "Replacing bare ccproxy with pipx-installed ccproxy-api[plugins-claude,plugins-codex]..."
        if install_ccproxy_via_pipx; then
          step_done "CCProxy ${tag_name} installed via pipx (full plugin set)"
          # Cleanup
          rm -f "$tmp_archive"
          local filtered=()
          for item in "${ROLLBACK_ITEMS[@]}"; do
          ...
          warn "pipx install failed; retry later with: pipx install ccproxy-api[plugins-claude,plugins-codex]"
        fi
      else
        warn "Install Python 3.11+ and pipx, then run: pipx install ccproxy-api[plugins-claude,plugins-codex]"
      fi
    fi
...
# Returns 0 (true) if 'ccproxy auth providers' reports at least one provider,
# 1 (false) if it returns "No OAuth providers found" or fails entirely.
test_ccproxy_has_providers() {
  local binary_path="$1"
  [[ ! -x "$binary_path" ]] && return 1
  
  local output
  output="$("$binary_path" auth providers 2>&1)" || return 1
  
  # Check for "No OAuth providers found" or "No plugins found"
  if echo "$output" | grep -qiE "no (oauth )?providers? found"; then
    return 1
  fi
  
  # Check for real provider names (non-empty lines that aren't headers)
  echo "$output" | grep -vE "^(Available|warning|Warning|Available OAuth Providers|providers found)$" | grep -q .
}
...
test_pipx_available() {
  ...
  local pyver
  pyver="$($py_cmd --version 2>&1 | grep -oE '[0-9]+\.[0-9]+' | head -1)"
  [[ -z "$pyver" ]] && return 1
...
# Install ccproxy-api with full plugin set via pipx, then create a symlink
# in the clawde bin dir as ccproxy. Returns 0 on success, 1 on failure.
install_ccproxy_via_pipx() {
  local py_cmd="python3"
  command -v python3 >/dev/null 2>&1 || py_cmd="python"
  
  info "Installing ccproxy-api with full plugin set via pipx..."
  if ! pipx install --python "$py_cmd" "ccproxy-api[plugins-claude,plugins-codex]" 2>&1; then
    # pipx returns non-zero on upgrade if already installed; try 'pipx upgrade' instead
    pipx upgrade ccproxy-api --python "$py_cmd" 2>&1 || return 1
  fi
  
  # Find the pipx-installed ccproxy
  local pipx_ccproxy
  pipx_ccproxy="$(command -v ccproxy)"
  if [[ -z "$pipx_ccproxy" ]]; then
    warn "pipx install succeeded but 'ccproxy' is not on PATH"
    return 1
  fi
  
  # Create symlink in CLAWDE_BIN_DIR
  local ccproxy_link="${CLAWDE_BIN_DIR}/ccproxy"
  rm -f "$ccproxy_link" 2>/dev/null || true
  if ln -s "$pipx_ccproxy" "$ccproxy_link" 2>/dev/null; then
    ok "CCProxy installed via pipx (full plugin set)"
    return 0
  else
    # Fallback: copy instead of symlink
    if cp "$pipx_ccproxy" "$ccproxy_link" 2>/dev/null && chmod +x "$ccproxy_link"; then
      ok "CCProxy installed via pipx (full plugin set, copied)"
      return 0
    fi
    warn "Failed to create ccproxy symlink or copy"
    return 1
  fi
}

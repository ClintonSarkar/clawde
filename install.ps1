...
function Install-CCProxy {
    ...
    # Determine platform-specific asset. Is64BitOperatingSystem is true on BOTH
    # x64 and ARM64, so it can't distinguish them - use PROCESSOR_ARCHITECTURE.
    switch ($env:PROCESSOR_ARCHITECTURE) {
        "ARM64" { $arch = "aarch64" }
        "AMD64" { $arch = "x86_64" }
        default { $arch = if ([Environment]::Is64BitOperatingSystem) { "x86_64" } else { "i686" } }
    }
    $assetName = "ccproxy-${tagName}-${arch}-pc-windows-msvc.zip"
    $asset = $release.assets | Where-Object { $_.name -eq $assetName } | Select-Object -First 1
    if (-not $asset) {
        Write-Err "No CCProxy binary published for Windows ${arch} in release ${tagName} (looked for ${assetName})`nAvailable assets: $($release.assets.name -join ', ')`nIf you are on ARM with no ARM build available, install via pipx: pipx install `"ccproxy-api[plugins-claude,plugins-codex]`""
        return
    }
...
[opencode]
# [active] auto_start controls boot/login auto-start of the proxy.
...
# ====================================================================
# Recursively convert a ConvertFrom-Json PSCustomObject (and nested arrays) into
# plain hashtables. Windows PowerShell 5.1 has no `ConvertFrom-Json -AsHashtable`,
# so we walk the object graph ourselves to get a mutable, indexable structure.
function ConvertTo-HashtableFromJson {
    param($InputObject)
    if ($null -eq $InputObject) { return $null }
    if ($InputObject -is [System.Collections.IEnumerable] -and $InputObject -isnot [string]) {
        $list = @()
        foreach ($item in $InputObject) { $list += ,(ConvertTo-HashtableFromJson $item) }
        # Comma-wrap the return so PowerShell does NOT unroll a single-element
        # array to a scalar - that would silently turn a user's ["one"] config
        # value back into "one" on write-back.
        return ,$list
    }
    if ($InputObject -is [System.Management.Automation.PSCustomObject]) {
        $ht = @{}
        foreach ($prop in $InputObject.PSObject.Properties) {
            $ht[$prop.Name] = ConvertTo-HashtableFromJson $prop.Value
        }
        return $ht
    }
    return $InputObject
...
function Setup-OpenCodeProvider {
    ...
    # Read any existing config as a mutable hashtable so we MERGE our provider
    # in rather than overwriting the user's other providers.
    #   - ConvertFrom-Json has NO -Depth parameter on Windows PowerShell 5.1
    #     (the documented minimum); passing it there throws and used to send us
    #     down the catch, which reset the config to empty and wiped the file.
    #   - ConvertFrom-Json returns a PSCustomObject; indexing it like a
    #     hashtable ($obj.provider["x"] = ...) fails. Convert to a hashtable
    #     first (ConvertFrom-Json -AsHashtable on PS6+, manual walk on 5.1).
    $existingConfig = @{}
    if (Test-Path $opencodeConfigFile) {
        $raw = Get-Content $opencodeConfigFile -Raw
        # An empty/whitespace-only file is a fresh start, not a parse error
        # (ConvertFrom-Json on $null raises a noisy binding error otherwise).
        if (-not [string]::IsNullOrWhiteSpace($raw)) {
            try {
                if ($PSVersionTable.PSVersion.Major -ge 6) {
                    $existingConfig = $raw | ConvertFrom-Json -AsHashtable
                } else {
                    $existingConfig = ConvertTo-HashtableFromJson ($raw | ConvertFrom-Json)
                }
            } catch {
                Write-Warn "Could not parse existing OpenCode config at $opencodeConfigFile; leaving it untouched and skipping provider setup"
                return
            }
            # Parseable but not a JSON object (e.g. an array or scalar root):
            # do NOT overwrite it - honor the same no-wipe contract as the catch.
            if ($null -eq $existingConfig -or $existingConfig -isnot [hashtable]) {
                Write-Warn "Existing OpenCode config at $opencodeConfigFile is not a JSON object; leaving it untouched and skipping provider setup"
                return
            }
        }
    }

    # Ensure provider section exists (as a hashtable we can index into)
    if (-not $existingConfig.ContainsKey("provider") -or $existingConfig["provider"] -isnot [hashtable]) {
        $existingConfig["provider"] = @{}
    }

    # Add/Update ONLY our provider; every other key the user had is preserved.
